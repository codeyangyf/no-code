...
| Task 4: 多租户上下文 | Task 4-1 ~ Task 4-5 | ⬜ 待实施 | - |
| Task 5: RBAC权限体系 | Task 5-1 ~ Task 5-6 | ⬜ 待实施 | - |
| Task 6: 对象存储与密钥管理 | Task 6-1 ~ Task 6-5 | ⬜ 待实施 | - |
| Task 7: 安全基线 | Task 7-1 ~ Task 7-5 | ⬜ 待实施 | - |
...
- Create: `backend/system-core/src/main/resources/db/migration/V13__audit_log_align.sql`（补齐设计文档要求的字段）
...
2. audit_log 表对齐：V13 迁移脚本 ALTER TABLE 补齐 `action, resource_type, resource_id, client_ip, result, error_message` 字段（保留旧字段兼容）
...
- [ ] **Task 7-1: 创建 AuditLog 注解 + V13 迁移脚本对齐字段 + AuditLogEntity 实体**
- [ ] **Task 7-2: 创建 AuditLogRepository + AuditLogService + AuditLogServiceImpl**
- [ ] **Task 7-3: 创建 AuditLogAspect AOP切面（需确保AOP依赖已引入）**
- [ ] **Task 7-4: 创建 FileUploadValidator + SsrfProtector + AuditLogController**
- [ ] **Task 7-5: 添加 CORS 配置 + 编译验证 + Commit**
...
### Task 7 验收
- [ ] @AuditLog 注解的方法自动记录审计日志到 audit_log 表
- [ ] 文件上传校验正确拦截非法文件（类型/大小/路径遍历）
- [ ] SSRF 防护能正确识别并拦截内网URL
- [ ] CORS 配置生效，前端可正常跨域请求
- [ ] 审计日志查询接口可用（分页查询、按条件筛选）
